...
pub async fn upload_and_convert_font(
    ...
    if !fonts_dir.exists() {
        fs::create_dir_all(&fonts_dir).map_err(|e| format!("创建字体目录失败: {}", e))?;
    }
    ...
    match file_ext.as_str() {
        "woff2" => {
            ...
            let metadata =
                fs::metadata(&dest_path).map_err(|e| format!("读取文件信息失败: {}", e))?;
        ...
        "ttf" => {
            ...
            let metadata =
                fs::metadata(&woff2_path).map_err(|e| format!("读取文件信息失败: {}", e))?;
...
pub async fn upload_font_data(
    ...
    fs::write(&temp_path, &data).map_err(|e| format!("保存临时文件失败: {}", e))?;
