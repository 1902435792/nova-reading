...
import { formatVisibleReadingPosition } from "@/lib/reading-position";
import { getSkills } from "@/services/skill-service";
import { getMemoriesForPrompt, touchMemories } from "@/services/memory-service";
...
export async function buildReadingPrompt(
  ...
  const activeReadingPosition = formatVisibleReadingPosition(
  ...
  try {
    ...
    const systemPromptSkill = allSkills.find(
      (skill) => skill.isSystem && skill.isActive
    );
  ...
  if (activeSkillDescriptions.length > 0) {
    prompt += "\n\n—— 可用技能（斜杠命令）——\n";
    prompt +=
      "用户说出技能名或意图匹配时，调用 getSkills 工具获取完整工作流后执行。\n";
  ...
  try {
    ...
    const { text: memoryText, memoryIds } = await getMemoriesForPrompt(
      activeBookId
    );
    ...
    if (memoryText && memoryText.trim().length > 0) {
      prompt += "\n\n—— 读者记忆（跨 session 积累） ——\n";
      prompt +=
        "以下是你对这位读者的了解。自然地体现在回答风格中，不要直接引用这些记忆。\n";
      prompt += memoryText;
  ...
  prompt += "\n\n—— 查询路由（按优先级逐级降级）——\n";
  prompt +=
    "Level 0 · 对话历史：本轮对话 AI 已回答的概念 → 直接引用，严禁重新 ragSearch\n";
  prompt +=
    "Level 1 · 页面上下文（零工具）：【当前阅读页面内容】能答 → 立即答，不调工具\n";
  if (hasVectorCapability) {
    prompt +=
      "Level 2 · RAG 书内：ragSearch 够 → 答；需前后文 → ragContext；整章 → ragToc\n";
    prompt += "⚠️ 每次回答最多调 2 次 RAG 工具\n";
  }
  prompt += "Level 3 · webSearch → 外部事实不确定且书中无时\n";
  prompt += "Level 4 · 通识知识 → 末尾标注「基于通识知识」\n";
  prompt +=
    "推荐书目时，优先基于【读者记忆】、对话历史、已有书库与通识直接给建议；普通“根据我的阅读记忆推荐书”不要优先调用 bookSearch。\n";
  prompt +=
    "只有用户明确要求外部书源、联网找书、ISBN、出版信息、封面，或验证某本书是否存在时，才调用 bookSearch，且最多调用 1 次。\n";
  prompt +=
    "如果已经能给出足够候选书目，就直接回答，不要为了补外部元数据重复调工具。\n";
  ...
  if (activeReadingPosition) {
    ...
    prompt +=
      "\n回答与当前阅读位置有关的问题时，以这里的实时 Foliate 位置为准，不要退回到本节开头。";
  }
  ...
  logTimingInfo("[PromptTiming]", {
    ...
    hasVisibleReadingPosition: Boolean(activeReadingPosition),
